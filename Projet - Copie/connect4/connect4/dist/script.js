...
function MAJ(){
  ...
  // message demandé : qui vient de jouer + à qui le tour
...
/* ===== PATCH: petit boost centre + heuristique déjà existante ===== */
...
function evaluation(t,maxP){
  ...
  for (let r=0;r<H();r++){
    ...
    if(t[r][centerCol]===maxP) score+=10; // + fort centre pour nous

    if(t[r][centerCol]===minP) score-=6;  // évite de donner le centre
...
/* ===== PATCH Minimax+: helpers tactiques ===== */

/** Gagne en 1 coup ? renvoie col ou null */
...
/** L’adversaire a une win en 1 dans t ? */
...
/** Jouer 'col' donne une win en 1 à l’adversaire ? */
...
/** Petit bonus pour 3‑ouvertes créées */
...
/* ===== PATCH Minimax+: robotMinimax sécurisé ===== */
...
function robotMinimax(jouerVraiment=true){
  ...
  if (winningNow != null){
    ...
    markBestScore(winningNow);

    statut.textContent = `Minimax+ : je gagne en 1 → colonne ${winningNow+1}`;
  ...
  // (B) Blocage si l’adversaire gagne en 1
  ...
  if (oppWinCol != null){
    ...
    markBestScore(oppWinCol);

    statut.textContent = `Minimax+ : je dois bloquer → colonne ${oppWinCol+1}`;
  ...
  // (C) Éviter les coups qui donnent une win en 1 à l’adversaire
  ...
  for (let c=0; c<L(); c++){
    ...
    if (!moveAllowsOppImmediateWin(t0, c, maxP)) safeMoves.push(c);
  ...
  // (D) Profondeur adaptative et bonus menaces

  const totalEmpty = t0.flat().filter(x=>x===null).length;

  const extraDepth = (totalEmpty <= 16) ? 2 : 0; // un peu plus profond en fin de partie
  ...
  for (const col of candidateMoves){
    ...
    // bonus menaces 3‑ouvertes

    const bonus = countThreat3Open(t0, maxP);



    // score minimax

    const val = minimax(t0, depth-1, false, maxP, -Infinity, +Infinity) + bonus;
...
async function robotDb(){
  ...
  // NOTE: on tente la DB même si paint.enabled (séquence potentiellement approximative).
...
function robotJoue(){
  ...
  if (IA.type === "db")      return robotDb();       // DB même si paint
...
async function analyserSansJouer(){
  ...
  // IA = DB : on tente même si paint.enabled === true (et on tombera en fallback si besoin)
...
/* === PATCH Option A : estimation automatique du tour après peinture === */
...
function estimateNextPlayerFromCounts(){
  ...
  if (r === j) return config.commence; // même nombre → le starter rejoue

  if (r === j + 1) return "jaune";     // Rouge a +1 → à Jaune

  if (j === r + 1) return "rouge";     // Jaune a +1 → à Rouge

  return null;                         // écart > 1 → on ne change pas
...
function paintApply(row, col){
  ...
  // pas de victoire auto en peinture
  ...
  // <<< Option A : recalcule automatiquement qui doit jouer maintenant
 ...
 * (imparfait si la peinture met des pions en l’air),

 * best effort : par gravité approximative.
